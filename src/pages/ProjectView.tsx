...
export function ProjectView() {
  ...
  const isCarousel =
    project.type === "carousel" || Array.isArray(project.image);
    ...
    <div className="min-h-screen flex flex-col bg-bg text-white">
      ...
      <div className="fixed top-0 left-0 right-0 h-20 bg-bg/80 backdrop-blur-md border-b border-white/10 z-50 flex items-center justify-between px-6 md:px-12">
        ...
        >
          ...
          <span className="font-roboto tracking-wider text-sm">
            BACK TO WORK
          </span>
        </button>

        <h1 className="font-Advercase text-xl hidden md:block">
          {project.title}
        </h1>
        ...
        {hasUrl ? (
          ...
          >
            {project.type === "poster" || project.type === "carousel"
              ? "VIEW PROJECT"
              : "VISIT LIVE"}
        ...
        >
          {isWebsite ? (
            ...
            <div className="w-full">
              <div
                className={`mx-auto flex flex-col items-center ${isFewImages ? "gap-0 py-0" : "w-[95%] md:w-[80%]"}`}
              >
                {images.map((img, index) => (
                  <motion.div
                    ...
                    className={
                      isFewImages ? "flex justify-center w-full" : "w-full"
                    }
                  >
                    <img
                      src={
                        img.startsWith("http")
                          ? img
                          : `https://raw.githubusercontent.com/rakeshkanna-rk/database/refs/heads/main/new_portfolio/${img.replace(/^\//, "")}`
                      }
                ...
                <div className="text-center py-12">
                  ...
                  {project.category && (
                    <p className="font-sugopro tracking-widest text-white/40 text-sm uppercase">
                      {project.category}
                    </p>
                  )}
                </div>
            ...
            <div className="w-full h-full flex flex-col items-center justify-center p-4">
              <img
                src={(() => {
                  const img = Array.isArray(project.image)
                    ? project.image[0]
                    : project.image;
              ...
              <div className="md:hidden mt-6 text-center">
                <h1 className="font-Advercase text-2xl mb-2">
                  {project.title}
                </h1>
                {project.category && (
                  <p className="font-sugopro tracking-widest text-white/40 text-xs">
                    {project.category}
                  </p>
                )}
              </div>
