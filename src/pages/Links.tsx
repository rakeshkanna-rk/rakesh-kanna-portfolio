...
import { Instagram, Linkedin, Mail, Twitter, Globe, Github } from "lucide-react";
...
export function Links() {
  return (
    <div className="min-h-screen flex flex-col pt-32 bg-bg">
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 md:px-0">
        <div className="text-center mb-16">
          ...
          >
            <div className="w-full h-full rounded-full bg-white/10 flex items-center justify-center overflow-hidden grayscale hover:grayscale-0 transition-all duration-500">
              <img src="/mergex_logo.svg" alt="Profile" className="w-12 h-12" />
            </div>
          </motion.div>
          <SectionHeader title="Connect with Me" className="flex justify-center" />
          <p className="text-white/50 font-roboto text-sm md:text-base mt-4 max-w-md mx-auto">
            Design Lead at Mergex. Building digital products with intent and visual clarity.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4 pb-24">
          {links.map((link, index) => (
            ...
            >
              <div className={`w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center transition-colors duration-300 ${link.color}`}>
                {link.icon}
              </div>
